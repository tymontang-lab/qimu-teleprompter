...
test("ships the local-first PWA resources", async () => {
  ...
  assert.match(serviceWorker, /qimu-v2/);
  ...
  assert.match(page, /type VideoRatio/);
  assert.match(page, /"original"/);
  assert.match(page, /"9:16"/);
  assert.match(page, /"3:4"/);
  assert.match(page, /"1:1"/);
  assert.match(page, /createProcessedRecordingStream/);
  assert.match(page, /requestWakeLock/);
  assert.match(page, /promptProgress/);
  assert.match(page, /live-prompt-controls/);
